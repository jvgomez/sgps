    ...
    for (vector<File>::iterator it (files.begin()); it!=files.end(); ++it) {
        ...
        ////NO NOAA File
                ...
                if (strcmp (s, "genericfile") == 0){
                    const vector<float> lim = opt->getLimits();
                    File * filePtr = new File(it->getPath(), lim[0], lim[1]);
                    validFiles.push_back(filePtr);
                }
                else if (strcmp (s, "genericdec") == 0){   //////MIRAR!!!!!!
                    const vector<float> lim = opt->getLimits();
                    File * filePtr = new FileDec(it->getPath(), lim[0], lim[1]);
                    validFiles.push_back(filePtr);
                }
...
Day DirAnalyzer::extractDay(string path){
    ...
    if (opt->isFileType("generic"))
    {
        if (default_vals)
            ...
            f = new File(path, lim[0], lim[1]);
    }
    else if (opt->isFileType("genericdec"))
    {
        if (default_vals)
            f = new FileDec(path);
        else
            f = new FileDec(path, lim[0], lim[1]);
    }
