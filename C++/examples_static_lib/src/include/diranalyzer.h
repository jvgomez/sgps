...
public:
	...
	std::vector<Day> readFiles (bool saveData = false);
	 ...
	 * @param namedir name of file to save in the directory created.
	 * */
	static std::string createDir(const std::string namedir);
	
	Day extractDay(const int day, const int year, std::string path);
